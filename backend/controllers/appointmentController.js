...
//helper 
//this funtion will return a finite number.
const safeNumber = (v) => {
  const n = Number(v);
...
export const createAppointment=async (req,res)=>{
    ...
    if(existingBooking){
        return res.status(409).json({
            success:false,
            message:"You are already have an appointment with this doctor at the selected slot."
        });
    }
    let doctor=null;
    try{
...
try {
    ...
    console.error("Stripe retrieve session", err);
    return res.status(404).json({
        success: false,
        message: "Stripe session not found"
    });
}
if(!session) returnres.status(404).json({
    success:false,
...
export const getAppointmentsByDoctor = async (req, res) => {
  ...
  .populate("doctorId", "name specialization owner imageUrl image")
  .lean();
  console.log("Appointments Found:", items);
